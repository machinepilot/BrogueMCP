...
// External function from dm_main.c
extern void open_narrator_settings(void);
extern void close_narrator_settings(void);

// Local state variable for the narrator UI
boolean narratorUIActive = false;
...
void mainInputLoop() {
    ...
    while (!rogue.gameHasEnded && (!playingBack || !canceled)) { // repeats until the game ends
        ...
        // Check if the narrator UI is active, if so, pause game loop
        if (narratorUIActive) {
            // Display a message that the game is paused
            message("Game paused while narrator UI is open...", false);
            
            // Wait a brief moment to not consume all CPU cycles
            pauseBrogue(100, PAUSE_BEHAVIOR_DEFAULT);
            
            // Check if the UI has been closed (this will be set externally)
            if (!narratorUIActive) {
                message("Narrator settings updated. Game resumed.", false);
            }
            
            // Skip the rest of the loop while UI is active
            continue;
        }
...
void executeKeystroke(signed long keystroke, boolean controlKey, boolean shiftKey) {
    ...
    switch (keystroke) {
        ...
        case NUMPAD_3:
            if (shiftKey) {
                // If Shift+N is pressed, launch the narrator UI
                launchNarratorUI();
                direction = -1; // Clear the direction so movement isn't triggered
            } else {
                direction = DOWNRIGHT;
            }
            break;
...
void launchNarratorUI() {
    // Set UI active state
    narratorUIActive = true;
    
    // Send command to MCP bridge to open UI in browser
    open_narrator_settings();
    
    // Display a message to the user
    message("Opening the Mirror of Galadriel narrator interface...", false);
}
