...
#include "mcp_compat.h"
#include <string.h>
#include <stdio.h>

// Store original function pointers for restoration
#ifdef BROGUE_CURRENT
static void (*original_display_message)(const char *, unsigned long) = NULL;
#else
...
#ifdef BROGUE_CURRENT
static void dm_enhanced_display_message(const char *msg, unsigned long flags) {
    // Call original function
    original_display_message(msg, flags);
...
void handle_monster_encountered(creature *monst) {
    ...
    get_location_description(location_desc, 100, MCP_LOC_X(monst), MCP_LOC_Y(monst));
    
    int is_first = is_first_encounter(monst->info.monsterID);
    char monsterName[100];
    strncpy(monsterName, monst->info.monsterName, sizeof(monsterName) - 1);
    monsterName[sizeof(monsterName) - 1] = '\0';
    
    snprintf(event_data, sizeof(event_data),
             "{\"monsterName\":\"%s\",\"monsterLevel\":%d,\"isRare\":%s,\"isFirstEncounter\":%s,\"locationDesc\":\"%s\"}",
             monsterName,
             MCP_MONSTER_LEVEL(monst),
             MCP_IS_MONSTER_RARE(monst),
             is_first ? "true" : "false",
             location_desc);
...
void handle_item_discovered(item *theItem) {
    ...
    // Create JSON event data
    char event_data[2048];
    char itemNameBuffer[100];
    
    // Get item name using compatibility function
    mcp_get_item_name(theItem, itemNameBuffer, sizeof(itemNameBuffer));
...
void handle_player_died(char *killedBy) {
    ...
    snprintf(event_data, sizeof(event_data),
             "{\"killedBy\":\"%s\",\"totalTurns\":%d,\"maxDepth\":%d}",
             killedBy,
             MCP_TURN_COUNT,
             rogue.deepestLevel);
...
void display_pending_dm_messages(void) {
    if (has_pending_message) {
        // Display the message
        MCP_DISPLAY_MESSAGE(dm_message_buffer);
